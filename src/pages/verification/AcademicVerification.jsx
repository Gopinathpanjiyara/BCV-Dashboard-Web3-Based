import React from 'react';
import { PlusIcon, MinusIcon } from '@heroicons/react/24/outline';

const AcademicVerification = ({ data, onChange, onFileUpload, onAddItem, onRemoveItem }) => {
  return (
    <div className="bg-background-lighter rounded-xl p-6 shadow-neumorph mb-6">
      <h2 className="text-xl font-medium mb-4">Academic Verification</h2>
      
      {data.degrees.map((degree, index) => (
        <div key={index} className="mb-6 border border-gray-700 rounded-xl p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Degree {index + 1}</h3>
            {index > 0 && (
              <button
                type="button"
                onClick={() => onRemoveItem('academic', 'degrees', index)}
                className="p-1 bg-red-500/20 text-red-400 rounded-full hover:bg-red-500/30 transition-colors"
              >
                <MinusIcon className="w-5 h-5" />
              </button>
            )}
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">Degree*</label>
              <input
                type="text"
                value={degree.degree}
                onChange={(e) => onChange('academic', 'degrees', { ...degree, degree: e.target.value }, index)}
                className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-white focus:ring-primary focus:border-primary"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Field of Study*</label>
              <input
                type="text"
                value={degree.field}
                onChange={(e) => onChange('academic', 'degrees', { ...degree, field: e.target.value }, index)}
                className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-white focus:ring-primary focus:border-primary"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Institution*</label>
              <input
                type="text"
                value={degree.institution}
                onChange={(e) => onChange('academic', 'degrees', { ...degree, institution: e.target.value }, index)}
                className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-white focus:ring-primary focus:border-primary"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Graduation Date*</label>
              <input
                type="date"
                value={degree.graduationDate}
                onChange={(e) => onChange('academic', 'degrees', { ...degree, graduationDate: e.target.value }, index)}
                className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-white focus:ring-primary focus:border-primary"
                required
              />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-1">Upload Degree Certificate</label>
              <input
                type="file"
                onChange={(e) => onFileUpload('academic', 'degrees', e, index)}
                className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-white focus:ring-primary focus:border-primary"
              />
              {degree.document && (
                <p className="mt-1 text-sm text-green-400">
                  File selected: {degree.document.name}
                </p>
              )}
            </div>
          </div>
        </div>
      ))}
      
      <button
        type="button"
        onClick={() => onAddItem('academic', 'degrees')}
        className="flex items-center gap-2 text-primary hover:text-primary-light"
      >
        <PlusIcon className="w-5 h-5" />
        <span>Add Another Degree</span>
      </button>
    </div>
  );
};

export default AcademicVerification;
